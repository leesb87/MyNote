...
.faq-edit-textarea {
	...
	function search(){
		...
		var keyword = $("[name=searchKeyword] [name=keyword]").val();
...
<body><center>
	...
	<table>
		<tr>
			<td align="center">
			<!-- 입력양식 관리 폼테그 파트 -->
			<form name="searchKeyword" method="post" action="${requestScope.croot}/dictList_kor.do">
		...
		<tr>
			...
			<span class="dictList">
			<!-- ------------------------------------------------------------------------------ -->
			<!-- DB에서 가져온 결과물을 꺼내기위해 Controller에 있는 ModerAndView에 저장한
			 dictList 객체 동시에 "HttpServletRequest에 저장되어 있는 것을 키값dictList로 꺼냄" -->
			<!-- ------------------------------------------------------------------------------ -->
				<!-- 한국어 -->
		...
		<c:if test="${empty requestScope.tabDictList_kor}">
